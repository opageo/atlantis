...
GFM processes Sentinel-1A/B SAR imagery in near real-time and emits a new
flood-extent map within hours of each acquisition. Effective global cadence is
~3–6 days depending on latitude (Sentinel-1A/B have ~6-day individual revisits;
joint coverage shortens the gap). Each acquisition contributes the native bands
below — of which `ensemble_flood_extent` and `reference_water_mask` are the two
primary Atlantis inputs:

| Asset                   | Meaning                                                                         |
| ----------------------- | ------------------------------------------------------------------------------- |
| `ensemble_flood_extent` | Flood classification: 0 = dry, 1 = flood, 255 = nodata                          |
| `ensemble_water_extent` | Open-water classification: 0 = dry, 1 = water (ensemble), 255 = nodata          |
| `reference_water_mask`  | Water type: 0 = no water, 1 = permanent water, 2 = seasonal water, 255 = nodata |
| `exclusion_mask`        | Native GFM exclusion codes (where SAR-based flood mapping is unreliable)        |
| `advisory_flags`        | Native GFM advisory bitmask (low regional backscatter / rough water)            |
| `ensemble_likelihood`   | Calibrated flood-likelihood value, 0–100 (255 = nodata)                         |

The native band inventory is maintained in the canonical
[GFM layer reference](../layers.md#layers-gfm) and surfaced via
...
### Ensemble flood extent

`ensemble_flood_extent` is the **primary binary flood mask** produced by the
Global Flood Monitoring (GFM) service. While
[`ensemble_likelihood`](#ensemble-likelihood) provides a probabilistic 0–100
scale, `ensemble_flood_extent` represents the actual spatial extent of
inundation determined by the majority-voting ensemble algorithm.

| Value | Meaning                                                            |
| ----- | ------------------------------------------------------------------ |
| `1`   | Flooded                                                            |
| `0`   | Not flooded (dry land)                                             |
| `255` | NoData (radar shadow, layover, extreme noise, or invalid coverage) |

The layer is delivered by the EODC STAC as a uint8 GeoTIFF and passed through
Atlantis untouched (nearest-neighbour resampling, masked-max aggregation). In
classified mode it is the foundation for the derived `flood_fraction` layer,
accumulated as `ensemble_flood_extent_count` across the date group.

### Ensemble water extent

`ensemble_water_extent` is the binary companion to `ensemble_flood_extent`,
covering **all open water observed in the acquisition** — flooded areas plus
any pre-existing (non-flood) surface water detected on that date. It is the
logical OR of the GFM Ensemble flood map with the permanent and seasonal water
from the Reference Water Mask (GFM PDD §4.2 / Table 19).

| Value | Meaning                                         |
| ----- | ----------------------------------------------- |
| `1`   | Water (flooded or pre-existing reference water) |
| `0`   | No water                                        |
| `255` | NoData                                          |

In classified mode it is the foundation for the derived `water_fraction`
layer (accumulated as `ensemble_water_extent_count` across the date group).
Note that the GFM **water** ensemble uses only the **LIST + DLR** algorithms
(TUW is excluded — it is designed to detect _temporary_ flooding only and is
unsuited to the broader water-mapping task — so `ensemble_water_extent` and
`ensemble_flood_extent` are produced by different algorithm combinations).

### Reference water mask

Unlike static land-cover water maps, the `reference_water_mask` accounts for
**seasonal hydrology**. It belongs to a fixed **5-year reference period covering
2017–2021** (Sentinel-1 A and B), generated by applying an ensemble water
mapping workflow (LIST + DLR) over the multi-year stack of SAR imagery. The
baseline methodology is static, but the delivered asset is selected from a
12-month family according to the acquisition month:

- **Permanent Water** (`1`): Computed from median SAR backscatter over the full
  5-year baseline — water present continuously.
- **Seasonal Water** (`2`): Provided as 12 monthly sub-masks, each mapping the
  baseline median water extent for a specific calendar month (e.g. wet/dry
  seasons, annual snowmelt).

| Value | Classification   | Meaning                                                         |
| ----- | ---------------- | --------------------------------------------------------------- |
| `0`   | Land / Non-Water | No baseline surface water                                       |
| `1`   | Permanent Water  | Water present continuously across the multi-year baseline       |
| `2`   | Seasonal Water   | Baseline water expected during the current calendar month only  |
| `255` | NoData / Masked  | Heavy topographic distortion, radar shadow, or missing coverage |

The layer is the **baseline** against which flood anomalies are measured — flood
extent is detected where SAR backscatter deviates from this reference. Atlantis
carries the native codes through as a companion layer in both classified
(`reference_water`) and native (`reference_water_mask`) mode.

### Ensemble likelihood

The `ensemble_likelihood` native layer encodes a **calibrated confidence score**
(0–100) produced by harmonising the three independent GFM flood-detection
algorithms — **LIST**, **DLR**, and **TUW** (TU Wien) — via the GFM Ensemble
flood-mapping algorithm (majority vote) into a single standardised scale
(50 = decision threshold; <50 unflooded, ≥50 flooded). The calibration method
is described in:
...
> _IEEE Journal of Selected Topics in Applied Earth Observations and Remote
> Sensing_, 2023.

The harmonised likelihood scale is:

| Value range  | Interpretation                       |
| ------------ | ------------------------------------ |
| Near **100** | High confidence flooded              |
| Near **50**  | Uncertain / decision threshold       |
| Near **0**   | High confidence unflooded / dry land |

Values of **255** indicate no data (nodata). Per GFM PDD §4.5, the
likelihood value is only defined for pixels **outside the GFM Exclusion Mask**
(inside the Exclusion Mask it is undefined / nodata). The likelihood band is
surfaced as a companion layer in both
[classified and native modes](../layers.md#layers-gfm-native); it is not
used in the derivation of `water_fraction` / `flood_fraction`.
