...
| `--inventory`                               | canonical catalogue (full history)   | canonical catalogue (full history)  | canonical catalogue (2025)          | Catalogue Parquet to ingest (see §3). Defaults to the canonical file — for a per-year cube pass the yearly file (`…_archive_catalog_<year>.parquet`), or the build streams every catalogue year's dates into the cube |
