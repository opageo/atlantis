...
*on top* of that store as a lightweight, serverless **discovery index**.
...
Every Collection and Item references the *same* Zarr store as an **asset**;
...
store. The Item describes a *time slice* of that store; the client opens the
...
hits = search_geoparquet("items.parquet", bbox=(-1.5, 38.8, 0.5, 40.0),
...
**datashader** and overlaid on a web-tile basemap with **geoviews**. This is the
...
serve_dashboard("viirs", archive_root="./data/viirs_2020_cube",
...
| `--basemap` | off | Web-tile basemap (requires `geoviews`) |
