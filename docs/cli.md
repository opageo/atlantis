...
| `--plot-dir`                           | `<output>/<source>/plots/` | Directory for PNG output. Nested as `processed/`or `harmonised/`, then `derived/` or `native/` (matching `--classify`/`--no-classify`) so re-runs never collide (filenames unchanged).                                                          |
...
| `--start`        | _(required)_                    | Start date `YYYY-MM-DD` (inclusive).   |
| `--end`          | _(required)_                    | End date `YYYY-MM-DD` (inclusive).     |
