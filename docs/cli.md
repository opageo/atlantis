...
| Option                                 | Default                    | Description                                                                                                                                         |
| -------------------------------------- | -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--classify/--no-classify`             | `--classify`               | Classify pixels into flood-fraction / quality-mask / permanent-water layers (MODIS adds recurring-flood). `--no-classify` writes raw integer codes. |
| `--stream/--no-stream`                 | `--stream`                 | Stream tiles via GDAL `/vsicurl/` vs. download to disk. MODIS: only valid with `--modis-backend lance_geotiff`.                                     |
| `--plot`                               | off                        | Save a PNG of the peak-flood date (VIIRS / MODIS / GFM).                                                                                            |
| `--plot-dir`                           | `<output>/<source>/plots/` | Directory for PNG output.                                                                                                                           |
| `--harmonise`                          | off                        | Harmonise the peak-flood date to 1 arcmin (VIIRS / MODIS / GFM).                                                                                    |
