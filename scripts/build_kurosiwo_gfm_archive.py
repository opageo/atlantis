"""Build the KuroSiwo events GFM Zarr archive.

Generates one task per (event, date, EQUI7 tile) for the full KuroSiwo event
set (windows = metadata range + 14-day post-flood pad), then streams them
...
def generate_tasks(aoi_table: pd.DataFrame) -> tuple[list[dict], list[dict]]:
    ...
    dropped: list[dict] = []
    if live_events:
        live_tasks, dropped = build_tasks_live(aoi_table, live_events, kurosiwo_task_id)
        tasks += live_tasks
...
def main() -> None:
    ...
    parser.add_argument(
        "--events",
        default="",
        help="comma-separated event ids (or event-aoi ids) to backfill",
    )
    parser.add_argument(
