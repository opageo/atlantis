"""Estimate 512×512-arcmin AOI blocks for every GEOID-Flood event-AoI.
...
maps each event-AoI to the origin-anchored 512×512-arcmin blocks covering its
bbox (:func:`atlantis.archive.grid.aoi_blocks`), and writes the AOI table to
``data/metadata/geoidflood_aois.csv`` with per-event date windows.

One row per ``(event, native AoI, block)`` — native AoI identity (the ``N``
in ``EMSR712-N``) is kept alongside the block id because two AoIs of one
activation can share a block; downstream task ids embed all three so nothing
collides.
...
number of batch tasks and the expected item load before any heavy processing.
...
def build_aoi_table(metadata: pd.DataFrame, pad_days: int = POST_FLOOD_PAD_DAYS) -> pd.DataFrame:
    """Map every GEOID-Flood event-AoI to its covering 512×512-arcmin blocks.

    Returns one row per (event, native AoI, block) with the block bbox (grid
    edge bounds, for STAC queries) and the event's date window.
    """
    rows: list[dict] = []
...
def count_items_per_aoi_date(table: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in table.itertuples(index=False):
        ...
        print(
            f"  {row.event_id}-{row.aoi_id} {row.block_id}: {len(hits)} items / "
            f"{hits['date'].nunique()} dates ({year} catalogue)"
        )
    return pd.DataFrame(rows)
...
def main() -> None:
    ...
    if args.with_items:
        ...
        table = table.merge(items, on=["event_id", "aoi_id", "block_id"], how="left")
    table.to_csv(OUTPUT_PATH, index=False)

    print(
        f"AOI table: {len(table)} (event, AoI, block) rows for "
        f"{table['event_id'].nunique()} events / "
        f"{table.groupby(['event_id', 'aoi_id']).ngroups} event-AoIs → {OUTPUT_PATH}"
    )
    print(f"  blocks per event-AoI: {table.groupby(['event_id', 'aoi_id']).size().describe().to_dict()}")
    ...
    if args.with_items:
        ...
        print(f"  catalogue item estimate available for {estimated.sum()}/{len(table)} (event, AoI, block) rows")
        if estimated.any():
            print(
