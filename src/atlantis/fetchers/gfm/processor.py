...
class GfmRasterProcessor:
    ...
    def process_items(
        ...
        *,
        ...
        logger.info(
            "Processing %d GFM items (coarsen=%d, resampling=%s)",
            len(items),
            self.coarsen_factor,
            self.resampling,
        )
        ...
        for idx, item in enumerate(items):
            try:
                ...
                logger.warning("Failed to load item %d (%s): %s", idx, item.id, e)
                continue
            ...
            logger.debug("Item %d/%d processed", idx + 1, len(items))

        if flood_count is None or valid_count is None:
            logger.warning("No valid data found in %d items", len(items))
            return None
    ...
    def _write_outputs(
        ...
        def _write_tif(data: np.ndarray, name: str, dtype: str, nodata) -> Path:
            ...
            with rasterio.open(str(path), "w", **profile) as dst:
                write_data = arr.copy()
                if dtype == "float32":
                    logger.info("Replacing NaN with nodata value %s for %s", nodata, name)
